...
con.execute(f"""
    ...
    SELECT
        ...
        list_extract(
            list_filter(nutriments, x -> x.name = 'energy'), 1
        )."100g" AS energy,

        list_extract(
            list_filter(nutriments, x -> x.name = 'energy-kcal'), 1
        )."100g" AS energy_kcal,

        list_extract(
            list_filter(nutriments, x -> x.name = 'proteins'), 1
        )."100g" AS proteins,

        list_extract(
            list_filter(nutriments, x -> x.name = 'carbohydrates'), 1
        )."100g" AS carbohydrates,

        list_extract(
            list_filter(nutriments, x -> x.name = 'sugars'), 1
        )."100g" AS sugars,

        list_extract(
            list_filter(nutriments, x -> x.name = 'fat'), 1
        )."100g" AS fat,

        list_extract(
            list_filter(nutriments, x -> x.name = 'saturated-fat'), 1
        )."100g" AS saturated_fat,

        list_extract(
            list_filter(nutriments, x -> x.name = 'fiber'), 1
        )."100g" AS fiber,

        list_extract(
            list_filter(nutriments, x -> x.name = 'salt'), 1
        )."100g" AS salt
